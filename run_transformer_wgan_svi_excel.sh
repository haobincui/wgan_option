...
TRAIN_CONFIG_TEMPLATE="${TRAIN_CONFIG_TEMPLATE:-configs/transformer_wgan/train_lp_aggressive.yaml}"
...
TRAIN_CONFIG_PATH="${WORK_DIR}/train_transformer_wgan.yaml"
GENERATE_CONFIG_PATH="${WORK_DIR}/generate_result_transformer_wgan.yaml"
...
emit("TEXT_MODE", payload.get("text_embedding_mode", "lp"))
...
TRAIN_OUTPUT_ROOT="${TRAIN_OUTPUT_ROOT:-${TRAIN_OUTPUT_ROOT_TEMPLATE:-outputs/training/transformer_wgan/${DATASET_FAMILY}}}"
if [[ -z "$TRAIN_OUTPUT_ROOT" ]]; then
  TRAIN_OUTPUT_ROOT="outputs/training/transformer_wgan/${DATASET_FAMILY}"
fi
GENERATE_OUTPUT_ROOT="${GENERATE_OUTPUT_ROOT:-outputs/generate_result/transformer_wgan/${DATASET_FAMILY}}"
...
python scripts/transformer_wgan/main.py train --config "$TRAIN_CONFIG_PATH" | tee -a "$LOG_PATH"

LATEST_TRAIN_RUN_DIR="$(find_latest_run_dir "$TRAIN_OUTPUT_ROOT")"
BEST_CHECKPOINT_PATH="${LATEST_TRAIN_RUN_DIR}/checkpoints/transformer_wgan_best.pt"
...
python scripts/transformer_wgan/main.py generate-result --config "$GENERATE_CONFIG_PATH" | tee -a "$LOG_PATH"

LATEST_GENERATE_RUN_DIR="$(find_latest_run_dir "$GENERATE_OUTPUT_ROOT")"

echo "=========================================="
echo "Transformer WGAN run complete"
echo "=========================================="
