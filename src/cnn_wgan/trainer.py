...
from .config import CnnWGANTrainConfig, save_config_yaml
...
class CnnWGANTrainer:
    """Train the standalone CNN WGAN model on `merged_vol.xlsx` rows."""

    def __init__(self, config: CnnWGANTrainConfig):
        self.config = config
    ...
    def logger(self) -> logging.Logger:
        if self._logger is None:
            logger = logging.getLogger("cnn_wgan.trainer")
