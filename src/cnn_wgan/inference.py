"""Scenario sampling and arbitrage-weighted aggregation for standalone CNN WGAN."""
...
from .config import CnnWGANSampleConfig, CnnWGANTrainConfig, save_config_yaml
from .data import (
    CnnWGANNormalizationStats,
    CnnWGANSample,
    denormalize_tensor,
    load_cnn_wgan_samples,
    normalize_surface_tensor,
    normalize_tensor,
    split_samples,
)
from .io import load_checkpoint, prepare_run_dir, write_csv, write_json
from .models import CnnWGANGenerator, reconstruct_future_surface
from .plotting import plot_cnn_wgan_payload
...
def normalization_stats_to_tensors(
    stats: CnnWGANNormalizationStats | Mapping[str, Any],
    device: torch.device,
) -> TensorNormalizationStats:
    if isinstance(stats, CnnWGANNormalizationStats):
        payload = {
...
class CnnWGANSampler:
    """Load a standalone CNN WGAN checkpoint and generate arbitrage-weighted scenarios."""

    def __init__(self, config: CnnWGANSampleConfig):
        self.config = config
    ...
    def _load_generator(self) -> tuple[CnnWGANGenerator, dict[str, Any], CnnWGANTrainConfig, TensorNormalizationStats]:
        if not str(self.config.checkpoint_path).strip():
            raise ValueError("checkpoint_path must be set for standalone CNN WGAN sampling.")
        checkpoint = load_checkpoint(self.config.checkpoint_path, self.device)
        train_config = CnnWGANTrainConfig(**checkpoint["config"])
    ...
    def sample(self):
        ...
        all_samples = load_cnn_wgan_samples(self.config)
        selected_samples = split_samples(self.config, all_samples)
        if not selected_samples:
            raise ValueError("No standalone CNN WGAN samples are available after split/selection.")
        ...
        for sample in selected_samples:
            ...
            if bool(self.config.save_plots):
                plot_cnn_wgan_payload(payload, self.plots_dir / f"{sample.global_index:04d}_{sample.sample_id}.png")
            summary_rows.append(
                {
                    "sample_id": sample.sample_id,
                    "global_index": int(sample.global_index),
