"""Standalone merged-vol workbook parsing for the CNN WGAN module."""
...
from .config import CnnWGANSampleConfig, CnnWGANTrainConfig
...
@dataclass(frozen=True)
class CnnWGANNormalizationStats:
    """Feature statistics used to normalize CNN WGAN inputs and outputs."""
...
@dataclass(frozen=True)
class CnnWGANDataBundle:
    """Train/validation loaders and metadata for the standalone CNN WGAN module."""
    ...
    maturity_days_grid: np.ndarray
    embedding_dim: int
    normalization_stats: CnnWGANNormalizationStats
    train_items: list[CnnWGANSample]
    val_items: list[CnnWGANSample]
    all_items: list[CnnWGANSample]
...
def _compute_normalization_stats(samples: Sequence[CnnWGANSample]) -> CnnWGANNormalizationStats:
    ...
    return CnnWGANNormalizationStats(
...
class CnnWGANDataset(Dataset):
    """Torch dataset yielding the tensors needed by the standalone CNN WGAN trainer."""

    def __init__(
        self,
        samples: Sequence[CnnWGANSample],
        normalization_stats: CnnWGANNormalizationStats,
        *,
...
def load_cnn_wgan_samples(config: CnnWGANTrainConfig | CnnWGANSampleConfig) -> list[CnnWGANSample]:
    ...
    for global_index, row in enumerate(rows):
        ...
        if strike_grid.size <= 0 or maturity_days_grid.size <= 0:
            raise ValueError("strike_grid and maturity_days_grid must be non-empty for standalone CNN WGAN.")
...
def _check_consistent_grids(samples: Sequence[CnnWGANSample]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for sample in samples[1:]:
        if sample.strike_grid.shape != strike_grid.shape or not np.allclose(sample.strike_grid, strike_grid):
            raise ValueError("All standalone CNN WGAN samples must share the same strike grid.")
        if sample.maturity_days_grid.shape != maturity_days_grid.shape or not np.allclose(
            sample.maturity_days_grid,
            maturity_days_grid,
        ):
            raise ValueError("All standalone CNN WGAN samples must share the same maturity grid.")
    return strike_grid.copy(), maturity_days_grid.copy()


def create_train_val_bundle(config: CnnWGANTrainConfig) -> CnnWGANDataBundle:
    samples = load_cnn_wgan_samples(config)
    if len(samples) < int(config.min_samples_for_training):
        raise ValueError(f"Standalone CNN WGAN requires at least {config.min_samples_for_training} samples.")
...
def select_samples(samples: Sequence[CnnWGANSample], *, selection_mode: str, selection_count: int) -> list[CnnWGANSample]:
...
def split_samples(config: CnnWGANSampleConfig, samples: Sequence[CnnWGANSample]) -> list[CnnWGANSample]:
    split_idx = _split_index(len(samples), config.train_ratio)
