...
def normalization_stats_to_tensors(
    stats: VolGANNormalizationStats | Mapping[str, Any],
    device: torch.device,
) -> TensorNormalizationStats:
    if isinstance(stats, VolGANNormalizationStats):
        payload = {
    ...
    if missing:
        raise ValueError(f"Checkpoint normalization_stats is missing keys: {missing}")
...
def prepare_condition_tensors(
    ...
    *,
    ...
    if normalize_current_surface:
        current_features = normalize_current_surface_tensor(
            current_flat,
            normalization.current_log_mean,
            normalization.current_log_std,
        )
    else:
        current_features = torch.log(torch.clamp(current_flat, min=1e-4))
...
def summarize_surface_scenarios(
    *,
    ...
    generated_tensor = torch.tensor(surface_stack, dtype=torch.float32)
...
class VolGANSampler:
    ...
    def _load_generator(self) -> tuple[VolGANGenerator, dict[str, Any], VolGANTrainConfig, TensorNormalizationStats]:
