"""Scenario sampling and arbitrage-weighted aggregation for standalone StyleMod WGAN."""
...
from .data import (
    StyleModWGANNormalizationStats,
    StyleModWGANSample,
    denormalize_tensor,
    load_stylemod_wgan_samples,
    normalize_surface_tensor,
    normalize_tensor,
    split_samples,
)
from .io import load_checkpoint, write_csv, write_json
from .models import StyleModWGANGenerator, reconstruct_future_surface
from .plotting import plot_stylemod_wgan_payload
...
def normalization_stats_to_tensors(
    stats: StyleModWGANNormalizationStats | Mapping[str, Any],
    device: torch.device,
) -> TensorNormalizationStats:
    if isinstance(stats, StyleModWGANNormalizationStats):
        payload = {
...
class StyleModWGANSampler:
    """Load a standalone StyleMod WGAN checkpoint and generate arbitrage-weighted scenarios."""

    def __init__(self, config: StyleModWGANSampleConfig):
        self.config = config
    ...
    def _load_generator(self) -> tuple[StyleModWGANGenerator, dict[str, Any], StyleModWGANTrainConfig, TensorNormalizationStats]:
        if not str(self.config.checkpoint_path).strip():
            raise ValueError("checkpoint_path must be set for standalone StyleMod WGAN sampling.")
        checkpoint = load_checkpoint(self.config.checkpoint_path, self.device)
        train_config = StyleModWGANTrainConfig(**checkpoint["config"])
        ...
        generator = StyleModWGANGenerator(
            ...
            style_dim=int(train_config.style_dim),
            style_noise_scale=float(train_config.style_noise_scale),
            style_demodulate=bool(train_config.style_demodulate),
        ).to(self.device)
    ...
    def sample(self):
        ...
        all_samples = load_stylemod_wgan_samples(self.config)
        selected_samples = split_samples(self.config, all_samples)
        if not selected_samples:
            raise ValueError("No standalone StyleMod WGAN samples are available after split/selection.")
        ...
        for sample in selected_samples:
            ...
            if bool(self.config.save_plots):
                plot_stylemod_wgan_payload(payload, self.plots_dir / f"{sample.global_index:04d}_{sample.sample_id}.png")
            summary_rows.append(
                {
                    "sample_id": sample.sample_id,
                    "global_index": int(sample.global_index),
