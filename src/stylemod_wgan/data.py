"""Standalone merged-vol workbook parsing for the StyleMod WGAN module."""
...
from .config import StyleModWGANSampleConfig, StyleModWGANTrainConfig
...
@dataclass(frozen=True)
class StyleModWGANNormalizationStats:
    """Feature statistics used to normalize StyleMod WGAN inputs and outputs."""
...
@dataclass(frozen=True)
class StyleModWGANDataBundle:
    """Train/validation loaders and metadata for the standalone StyleMod WGAN module."""
    ...
    maturity_days_grid: np.ndarray
    embedding_dim: int
    normalization_stats: StyleModWGANNormalizationStats
    train_items: list[StyleModWGANSample]
    val_items: list[StyleModWGANSample]
    all_items: list[StyleModWGANSample]
...
def _compute_normalization_stats(samples: Sequence[StyleModWGANSample]) -> StyleModWGANNormalizationStats:
    ...
    return StyleModWGANNormalizationStats(
...
class StyleModWGANDataset(Dataset):
    """Torch dataset yielding the tensors needed by the standalone StyleMod WGAN trainer."""

    def __init__(
        self,
        samples: Sequence[StyleModWGANSample],
        normalization_stats: StyleModWGANNormalizationStats,
        *,
...
def load_stylemod_wgan_samples(config: StyleModWGANTrainConfig | StyleModWGANSampleConfig) -> list[StyleModWGANSample]:
    ...
    parsed: list[StyleModWGANSample] = []
    for global_index, row in enumerate(rows):
        ...
        if strike_grid.size <= 0 or maturity_days_grid.size <= 0:
            raise ValueError("strike_grid and maturity_days_grid must be non-empty for standalone StyleMod WGAN.")
    ...
    for index, sample in enumerate(parsed):
        parsed[index] = StyleModWGANSample(
...
def _check_consistent_grids(samples: Sequence[StyleModWGANSample]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for sample in samples[1:]:
        if sample.strike_grid.shape != strike_grid.shape or not np.allclose(sample.strike_grid, strike_grid):
            raise ValueError("All standalone StyleMod WGAN samples must share the same strike grid.")
        if sample.maturity_days_grid.shape != maturity_days_grid.shape or not np.allclose(
            sample.maturity_days_grid,
            maturity_days_grid,
        ):
            raise ValueError("All standalone StyleMod WGAN samples must share the same maturity grid.")
    return strike_grid.copy(), maturity_days_grid.copy()


def create_train_val_bundle(config: StyleModWGANTrainConfig) -> StyleModWGANDataBundle:
    samples = load_stylemod_wgan_samples(config)
    if len(samples) < int(config.min_samples_for_training):
        raise ValueError(f"Standalone StyleMod WGAN requires at least {config.min_samples_for_training} samples.")
...
def select_samples(samples: Sequence[StyleModWGANSample], *, selection_mode: str, selection_count: int) -> list[StyleModWGANSample]:
...
def split_samples(config: StyleModWGANSampleConfig, samples: Sequence[StyleModWGANSample]) -> list[StyleModWGANSample]:
    split_idx = _split_index(len(samples), config.train_ratio)
