"""Typed configuration loaders for the standalone Cross-Attention WGAN module."""
...
@dataclass
class CrossAttnWGANTrainConfig:
    """Training configuration for the standalone Cross-Attention WGAN module."""
...
@dataclass
class CrossAttnWGANSampleConfig:
    """Generate-result configuration for the standalone Cross-Attention WGAN module."""
...
_TRAIN_DEFAULTS = CrossAttnWGANTrainConfig()
_SAMPLE_DEFAULTS = CrossAttnWGANSampleConfig()
_TRAIN_FIELD_NAMES = {field.name for field in fields(CrossAttnWGANTrainConfig)}
_SAMPLE_FIELD_NAMES = {field.name for field in fields(CrossAttnWGANSampleConfig)}
...
def default_train_output_root(data_path: str | Path) -> str:
    ...
    return str(default_output_root("outputs/training/crossattn_wgan", data_path))
...
def build_sample_config(
    *,
    ...
    if training_values:
        ...
        if unknown_training:
            raise ValueError(
                f"Unknown training config keys for Cross-Attention WGAN: {unknown_training}. "
    ...
    if generate_values:
        ...
        if unknown_generate:
            raise ValueError(
                f"Unknown generate_result config keys for Cross-Attention WGAN: {unknown_generate}. "
    ...
    if overrides:
        ...
        if unknown_overrides:
            raise ValueError(
                f"Unknown generate_result overrides for Cross-Attention WGAN: {unknown_overrides}. "
                f"Allowed keys: {sorted(_SAMPLE_FIELD_NAMES)}"
            )
        loaded_values.update(dict(overrides))

    config = CrossAttnWGANSampleConfig(**loaded_values)
...
def load_train_config(
    ...
    config = _coerce_config(CrossAttnWGANTrainConfig, loaded_values, source=str(config_path))
