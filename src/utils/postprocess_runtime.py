...
import numpy as np
from utils.output_paths import prepare_run_dir
from utils.training_paths import generate_result_dir
...
def prepare_run_output_dir(output_root: str | Path) -> Path:
    ...
    return prepare_run_dir(output_root, create=True)


def prepare_generate_output_dir(run_dir: str | Path, output_dir: str | Path = "") -> Path:
    """Resolve the fixed generate-result directory under one training run."""

    return generate_result_dir(run_dir, output_dir)
