"""Scenario sampling and result aggregation for standalone Transformer WGAN."""
...
from .config import TransformerWGANSampleConfig, TransformerWGANTrainConfig, save_config_yaml
from .data import (
    TransformerWGANNormalizationStats,
    TransformerWGANSample,
    denormalize_tensor,
    load_transformer_wgan_samples,
    normalize_tensor,
    split_samples,
)
from .io import load_checkpoint, prepare_run_dir, write_csv, write_json
from .losses import reweight_scenarios, total_arbitrage_penalty
from .models import TransformerWGANGenerator, reconstruct_future_surface
from .plotting import plot_transformer_wgan_payload
...
def normalization_stats_to_tensors(
    stats: TransformerWGANNormalizationStats | Mapping[str, Any],
    device: torch.device,
) -> TensorNormalizationStats:
    if isinstance(stats, TransformerWGANNormalizationStats):
        payload = {
    ...
    return TensorNormalizationStats(
        current_mean=_to_device_row(payload["current_mean"], device),
        current_std=_to_device_row(payload["current_std"], device),
...
def prepare_condition_tensors(
    ...
    *,
    ...
    if normalize_current_surface:
        current_features_flat = normalize_tensor(current_flat, normalization.current_mean, normalization.current_std)
    else:
        current_features_flat = current_flat
...
class TransformerWGANSampler:
    """Load a standalone Transformer WGAN checkpoint and generate result artifacts."""

    def __init__(self, config: TransformerWGANSampleConfig):
        self.config = config
    ...
    def _load_generator(
        self,
    ) -> tuple[TransformerWGANGenerator, dict[str, Any], TransformerWGANTrainConfig, TensorNormalizationStats]:
        if not str(self.config.checkpoint_path).strip():
            raise ValueError("checkpoint_path must be set for standalone Transformer WGAN sampling.")
        checkpoint = load_checkpoint(self.config.checkpoint_path, self.device)
        train_config = TransformerWGANTrainConfig(**checkpoint["config"])
        ...
        generator = TransformerWGANGenerator(
            ...
            model_dim=int(train_config.model_dim),
            layers=int(train_config.gen_layers),
            num_heads=int(train_config.num_heads),
            ffn_dim=int(train_config.ffn_dim),
            dropout=float(train_config.dropout),
            text_hidden_dim=int(train_config.text_hidden_dim),
            text_token_dim=int(train_config.text_token_dim),
            noise_hidden_dim=int(train_config.noise_hidden_dim),
        ).to(self.device)
    ...
    def sample(self):
        ...
        all_samples = load_transformer_wgan_samples(self.config)
        selected_samples = split_samples(self.config, all_samples)
        if not selected_samples:
            raise ValueError("No standalone Transformer WGAN samples are available after split/selection.")
        ...
        for sample in selected_samples:
            ...
            if bool(self.config.save_plots):
                plot_transformer_wgan_payload(payload, self.plots_dir / f"{sample.global_index:04d}_{sample.sample_id}.png")
            summary_rows.append(
                {
                    "sample_id": sample.sample_id,
                    "global_index": int(sample.global_index),
