"""Standalone merged-vol workbook parsing for the Transformer WGAN module."""
...
from .config import TransformerWGANSampleConfig, TransformerWGANTrainConfig
...
@dataclass(frozen=True)
class TransformerWGANNormalizationStats:
    """Feature statistics used to normalize Transformer WGAN inputs and outputs."""
...
@dataclass(frozen=True)
class TransformerWGANDataBundle:
    """Train/validation loaders and metadata for the standalone Transformer WGAN module."""
    ...
    maturity_days_grid: np.ndarray
    embedding_dim: int
    normalization_stats: TransformerWGANNormalizationStats
    train_items: list[TransformerWGANSample]
    val_items: list[TransformerWGANSample]
    all_items: list[TransformerWGANSample]
...
def _compute_normalization_stats(samples: Sequence[TransformerWGANSample]) -> TransformerWGANNormalizationStats:
    ...
    for sample in samples:
        ...
        current_surfaces.append(current_flat)
        deltas.append((target_flat - current_flat).astype(np.float32))
        text_embeddings.append(sample.text_embedding.astype(np.float32))
    current_arr = np.stack(current_surfaces, axis=0)
    delta_arr = np.stack(deltas, axis=0)
    text_arr = np.stack(text_embeddings, axis=0)
    return TransformerWGANNormalizationStats(
        current_mean=np.mean(current_arr, axis=0).astype(np.float32),
        current_std=_safe_std(current_arr),
        delta_mean=np.mean(delta_arr, axis=0).astype(np.float32),
        delta_std=_safe_std(delta_arr),
...
class TransformerWGANDataset(Dataset):
    """Torch dataset yielding the tensors needed by the standalone Transformer WGAN trainer."""

    def __init__(
        self,
        samples: Sequence[TransformerWGANSample],
        normalization_stats: TransformerWGANNormalizationStats,
        *,
    ...
    def __getitem__(self, index: int):
        ...
        current_flat = sample.current_surface.astype(np.float32).reshape(-1)
        target_flat = sample.target_surface.astype(np.float32).reshape(-1)
        target_delta = (target_flat - current_flat).astype(np.float32)
        current_features = current_flat.astype(np.float32)
        ...
        if self.normalize_current_surface:
            current_features = ((current_features - stats.current_mean) / stats.current_std).astype(np.float32)
...
def load_transformer_wgan_samples(config: TransformerWGANTrainConfig | TransformerWGANSampleConfig) -> list[TransformerWGANSample]:
    ...
    parsed: list[TransformerWGANSample] = []
    for global_index, row in enumerate(rows):
        ...
        if strike_grid.size <= 0 or maturity_days_grid.size <= 0:
            raise ValueError("strike_grid and maturity_days_grid must be non-empty for standalone Transformer WGAN.")
    ...
    for index, sample in enumerate(parsed):
        parsed[index] = TransformerWGANSample(
...
def _check_consistent_grids(samples: Sequence[TransformerWGANSample]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for sample in samples[1:]:
        if sample.strike_grid.shape != strike_grid.shape or not np.allclose(sample.strike_grid, strike_grid):
            raise ValueError("All standalone Transformer WGAN samples must share the same strike grid.")
        if sample.maturity_days_grid.shape != maturity_days_grid.shape or not np.allclose(
            sample.maturity_days_grid,
            maturity_days_grid,
        ):
            raise ValueError("All standalone Transformer WGAN samples must share the same maturity grid.")
    return strike_grid.copy(), maturity_days_grid.copy()


def create_train_val_bundle(config: TransformerWGANTrainConfig) -> TransformerWGANDataBundle:
    samples = load_transformer_wgan_samples(config)
    if len(samples) < int(config.min_samples_for_training):
        raise ValueError(f"Standalone Transformer WGAN requires at least {config.min_samples_for_training} samples.")
    ...
    return TransformerWGANDataBundle(
...
def select_samples(samples: Sequence[TransformerWGANSample], *, selection_mode: str, selection_count: int) -> list[TransformerWGANSample]:
...
def split_samples(config: TransformerWGANSampleConfig, samples: Sequence[TransformerWGANSample]) -> list[TransformerWGANSample]:
    split_idx = _split_index(len(samples), config.train_ratio)
