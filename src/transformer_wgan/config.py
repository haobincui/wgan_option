...
class TransformerWGANSampleConfig:
    """Generate-result configuration for the standalone Transformer WGAN module."""
...
_TRAIN_DEFAULTS = TransformerWGANTrainConfig()
_SAMPLE_DEFAULTS = TransformerWGANSampleConfig()
_TRAIN_FIELD_NAMES = {field.name for field in fields(TransformerWGANTrainConfig)}
_SAMPLE_FIELD_NAMES = {field.name for field in fields(TransformerWGANSampleConfig)}
...
def default_train_output_root(data_path: str | Path) -> str:
    ...
    return str(default_output_root("outputs/training/transformer_wgan", data_path))
...
def build_sample_config(
    *,
    ...
    if training_values:
        ...
        if unknown_training:
            raise ValueError(
                f"Unknown training config keys for Transformer WGAN: {unknown_training}. "
    ...
    if generate_values:
        ...
        if unknown_generate:
            raise ValueError(
                f"Unknown generate_result config keys for Transformer WGAN: {unknown_generate}. "
    ...
    if overrides:
        ...
        if unknown_overrides:
            raise ValueError(
                f"Unknown generate_result overrides for Transformer WGAN: {unknown_overrides}. "
                f"Allowed keys: {sorted(_SAMPLE_FIELD_NAMES)}"
            )
        loaded_values.update(dict(overrides))

    config = TransformerWGANSampleConfig(**loaded_values)
...
def load_train_config(
    ...
    config = _coerce_config(TransformerWGANTrainConfig, loaded_values, source=str(config_path))
