#!/usr/bin/env bash
# Run the RQ3 news-event vs no-news quiet post-processing workflow in background.
#
# Default:
#   CUDA_VISIBLE_DEVICES=1 bash scripts/rq3/run_news_quiet_background.sh
#
# Optional reuse of existing all-surface JSONs:
#   SVI_ALL_JSON=data/processed/svi-all/<run_ts>/surface-svi-all.json \
#   RAW_ALL_JSON=data/processed/raw-all/<run_ts>/surface-raw-all.json \
#   CUDA_VISIBLE_DEVICES=1 bash scripts/rq3/run_news_quiet_background.sh
...
OUT_ROOT="${OUT_ROOT:-outputs/rq3/rq3_news_quiet_${RUN_TS}}"
LOG_DIR="${LOG_DIR:-outputs/rq3/logs}"
DRIVER_LOG="${DRIVER_LOG:-${LOG_DIR}/rq3_news_quiet_${RUN_TS}.log}"
DRIVER_PID_FILE="${DRIVER_PID_FILE:-${LOG_DIR}/rq3_news_quiet_${RUN_TS}.pid}"
STATUS_FILE="${STATUS_FILE:-${LOG_DIR}/rq3_news_quiet_${RUN_TS}.status}"

if [[ "${RQ3_NEWS_QUIET_CHILD:-0}" != "1" ]]; then
  mkdir -p "${LOG_DIR}"
  if [[ -f "${DRIVER_PID_FILE}" ]]; then
    old_pid="$(cat "${DRIVER_PID_FILE}" 2>/dev/null || true)"
    if [[ -n "${old_pid}" ]] && kill -0 "${old_pid}" 2>/dev/null; then
      echo "RQ3 news/quiet workflow is already running: pid=${old_pid}"
      echo "Log: ${DRIVER_LOG}"
      exit 0
    fi
  fi

  RQ3_NEWS_QUIET_CHILD=1 \
  RUN_TS="${RUN_TS}" \
  OUT_ROOT="${OUT_ROOT}" \
  LOG_DIR="${LOG_DIR}" \
  ...
  SVI_ALL_JSON="${SVI_ALL_JSON:-}" \
  RAW_ALL_JSON="${RAW_ALL_JSON:-}" \
  ...
  echo "RQ3 news/quiet workflow started in background."
...
SVI_SURFACE_RUN_TS="rq3_news_quiet_svi_all_${RUN_TS}"
RAW_SURFACE_RUN_TS="rq3_news_quiet_raw_all_${RUN_TS}"
SVI_ALL_JSON="${SVI_ALL_JSON:-data/processed/svi-all/${SVI_SURFACE_RUN_TS}/surface-svi-all.json}"
RAW_ALL_JSON="${RAW_ALL_JSON:-data/processed/raw-all/${RAW_SURFACE_RUN_TS}/surface-raw-all.json}"

SVI_WORKBOOK="${OUT_ROOT}/svi_news_quiet_eval_workbook.xlsx"
RAW_WORKBOOK="${OUT_ROOT}/raw_vol_news_quiet_eval_workbook.xlsx"
GEN_DIR="rq3_news_quiet_${RUN_TS}_all_json"
...
log_step "START RQ3 news-event vs no-news quiet workflow"
...
log_step "FORCE_SURFACE=${FORCE_SURFACE}"
log_step "FORCE_GENERATE=${FORCE_GENERATE}"
...
if [[ ! -f "${SVI_ALL_JSON}" || "${FORCE_SURFACE}" == "1" ]]; then
  log_step "Generate SVI all-surface JSON"
  ...
  log_step "Reuse SVI all-surface JSON: ${SVI_ALL_JSON}"
fi
require_file "${SVI_ALL_JSON}" "SVI all-surface JSON"

if [[ ! -f "${RAW_ALL_JSON}" || "${FORCE_SURFACE}" == "1" ]]; then
  log_step "Generate raw-vol all-surface JSON"
  conda run -n "${ENV_NAME}" python scripts/generate_surface/main.py generate_surface \
    --config configs/surface_builder/raw/generate_surface-raw-all.yaml \
    --model raw \
    --data_range all \
    --run-ts "${RAW_SURFACE_RUN_TS}"
else
  log_step "Reuse raw-vol all-surface JSON: ${RAW_ALL_JSON}"
fi
require_file "${RAW_ALL_JSON}" "raw-vol all-surface JSON"

log_step "Build SVI news/quiet workbook"
conda run -n "${ENV_NAME}" python scripts/rq3/main.py build-news-quiet-workbook \
  --source-merged-vol "${SVI_SOURCE_WORKBOOK}" \
  --surface-all-json "${SVI_ALL_JSON}" \
  --news-xlsx "${NEWS_XLSX}" \
...
log_step "Build raw-vol news/quiet workbook"
conda run -n "${ENV_NAME}" python scripts/rq3/main.py build-news-quiet-workbook \
  --source-merged-vol "${RAW_SOURCE_WORKBOOK}" \
  --surface-all-json "${RAW_ALL_JSON}" \
  --news-xlsx "${NEWS_XLSX}" \
