"""Shared config parsing and runtime helpers for the daily-surface job."""
...
from quantlib.vol_surface.builder import OptionSurfaceBuilder, OptionSurfaceBuilderConfig
...
def load_daily_surface_config(config_path_value: str) -> Dict[str, Any]:
...
def parse_daily_surface_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    if "-h" in argv_list or "--help" in argv_list:
        ...
        config_defaults = load_daily_surface_config(pre_args.config)

    default_save_daily_csv = parse_bool_from_config(
...
def build_daily_surface_config(args: argparse.Namespace) -> OptionSurfaceBuilderConfig:
    return OptionSurfaceBuilderConfig(
...
def run_daily_surface_job(args: argparse.Namespace) -> Path:
    builder = OptionSurfaceBuilder(build_daily_surface_config(args))
    return builder.save(
