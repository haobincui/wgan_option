#!/usr/bin/env bash
# Launch the RQ2 ChatGPT-sentiment FiLM WGAN on raw-vol interpolation surfaces.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/training/film_wgan_raw_vol/llm_sentiment}"
ENV_NAME="${ENV_NAME:-py312}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}"
RUN_TS="${RUN_TS:-raw_vol_llm_sentiment_$(date -u +%Y%m%d-%H%M%S)}"
...
echo "Raw-vol RQ2 LLM sentiment FiLM WGAN launched"
echo "PID: ${PID}"
echo "Log: ${LOG_FILE}"
