...
if [[ ! -f "${SELECTED}" ]]; then
  echo "Selected checkpoint registry not found: ${SELECTED}" >&2
  exit 1
fi
...
PY
  ...
  if [[ "${FORCE_GENERATE}" != "1" && "${sample_count}" == "3711" && -f "${output_dir}/summary.csv" && -f "${output_dir}/run_metadata.json" && -f "${output_dir}/generate_resolved_config.yaml" ]]; then
    echo "Reusing existing generate-result output for ${model} seed ${seed}: ${output_dir}"
    continue
  fi

  echo "Generating all-split JSON for ${model} seed ${seed}"
