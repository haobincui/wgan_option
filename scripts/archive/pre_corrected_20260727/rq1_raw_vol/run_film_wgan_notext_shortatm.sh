#!/usr/bin/env bash
# Launch the RQ1 no-text FiLM WGAN on raw-vol interpolation surfaces.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/training/film_wgan_raw_vol/no_text}"
ENV_NAME="${ENV_NAME:-py312}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}"
RUN_TS="${RUN_TS:-raw_vol_notext_shortatm_$(date -u +%Y%m%d-%H%M%S)}"
...
echo "Raw-vol RQ1 no-text FiLM WGAN launched"
echo "PID: ${PID}"
echo "Log: ${LOG_FILE}"
