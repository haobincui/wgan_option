...
WORK_DIR="${WORK_DIR:-logs/stylemod_wgan/${RUN_TS}}"
...
if [[ -z "${TRAIN_CONFIG_PATH}" ]]; then
  ...
  echo "Example: $0 configs/stylemod_wgan/train_lp_gen128_disc128.yaml" >&2
  exit 1
fi
...
nohup python scripts/stylemod_wgan/main.py train --config "${TRAIN_CONFIG_PATH}" "$@" >"${LOG_FILE}" 2>&1 &
PID=$!
echo "${PID}" > "${PID_FILE}"

echo "=========================================="
echo "  StyleMod WGAN job launched"
echo "=========================================="
