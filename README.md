...
| `generate-result film-wgan` | `python scripts/film_wgan/main.py generate-result --config ...` | Generate scenarios from a trained Film WGAN checkpoint | `merged_vol.xlsx` plus a saved checkpoint | JSON payloads, plots, `summary.csv`, and fixed `generate_result/atm_vol/` ATM-vol CSV/PNG outputs | Standalone pipeline |
