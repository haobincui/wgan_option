...
TRAIN_CONFIG_TEMPLATE="${TRAIN_CONFIG_TEMPLATE:-configs/cnn_wgan/train_lp_aggressive.yaml}"
...
WORK_DIR="${WORK_DIR:-logs/cnn_wgan/${RUN_TS}}"
TRAIN_CONFIG_PATH="${WORK_DIR}/train_cnn_wgan.yaml"
GENERATE_CONFIG_PATH="${WORK_DIR}/generate_result_cnn_wgan.yaml"
...
TRAIN_OUTPUT_ROOT="${TRAIN_OUTPUT_ROOT:-${TRAIN_OUTPUT_ROOT_TEMPLATE:-outputs/training/cnn_wgan/${DATASET_FAMILY}}}"
if [[ -z "$TRAIN_OUTPUT_ROOT" ]]; then
  TRAIN_OUTPUT_ROOT="outputs/training/cnn_wgan/${DATASET_FAMILY}"
fi
GENERATE_OUTPUT_ROOT="${GENERATE_OUTPUT_ROOT:-outputs/generate_result/cnn_wgan/${DATASET_FAMILY}}"
...
python scripts/cnn_wgan/main.py train --config "$TRAIN_CONFIG_PATH" | tee -a "$LOG_PATH"

LATEST_TRAIN_RUN_DIR="$(find_latest_run_dir "$TRAIN_OUTPUT_ROOT")"
BEST_CHECKPOINT_PATH="${LATEST_TRAIN_RUN_DIR}/checkpoints/cnn_wgan_best.pt"
...
python scripts/cnn_wgan/main.py generate-result --config "$GENERATE_CONFIG_PATH" | tee -a "$LOG_PATH"

LATEST_GENERATE_RUN_DIR="$(find_latest_run_dir "$GENERATE_OUTPUT_ROOT")"

echo "=========================================="
echo "CNN WGAN run complete"
echo "=========================================="
