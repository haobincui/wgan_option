...
if [[ -z "${TRAIN_CONFIG_PATH}" ]]; then
  ...
  echo "Example: $0 configs/cnn_wgan/train_lp_gen128_disc128.yaml" >&2
  exit 1
fi
...
nohup python scripts/cnn_wgan/main.py train --config "${TRAIN_CONFIG_PATH}" "$@" >"${LOG_FILE}" 2>&1 &
PID=$!
echo "${PID}" > "${PID_FILE}"

echo "=========================================="
echo "  CNN WGAN job launched"
echo "=========================================="
