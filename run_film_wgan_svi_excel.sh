...
if [[ -z "${TRAIN_CONFIG_PATH}" ]]; then
  ...
  echo "Example: $0 configs/film_wgan/train_lp_gen128_disc128.yaml" >&2
  exit 1
fi
...
nohup python scripts/film_wgan/main.py train --config "${TRAIN_CONFIG_PATH}" "$@" >"${LOG_FILE}" 2>&1 &
PID=$!
echo "${PID}" > "${PID_FILE}"

echo "=========================================="
echo "  Film WGAN job launched"
echo "=========================================="
