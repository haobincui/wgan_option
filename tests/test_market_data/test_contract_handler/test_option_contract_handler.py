...
class TestOptionContractHandler(unittest.TestCase):
    ...
    def test_option_contract_handler_ty_three_digit_strikes(self):
        contracts = {
            'TY128N2': 128,
            'TY110O26': 110,
            'TY115C26': 115,
        ...
        for contract_name, expected_strike in contracts.items():
            with self.subTest(contract_name=contract_name):
                contract_handler = OptionContract(
                    contract_name=contract_name,
                    contract_type=ContractType.Option,
                )
                self.assertEqual(contract_handler.get_underlying().upper(), 'TY')
                self.assertEqual(contract_handler.get_strike(), expected_strike)
